...
class DropBlock(nn.Module):
    ...
    def _compute_block_mask(self, mask):
        ...
        non_zero_ids = mask.nonzero()
        nr_blocks = non_zero_ids.shape[0]
        ...
        if nr_blocks > 0:
            non_zero_ids = non_zero_ids.repeat(self.block_size ** 2, 1)
            offsets = offsets.repeat(nr_blocks, 1).view(-1, 4)
            offsets = offsets.long()

            block_ids = non_zero_ids + offsets
            padded_mask = F.pad(mask, (left_padding, right_padding, left_padding, right_padding))
            padded_mask[block_ids[:, 0], block_ids[:, 1], block_ids[:, 2], block_ids[:, 3]] = 1.
        else:
            padded_mask = F.pad(mask, (left_padding, right_padding, left_padding, right_padding))
