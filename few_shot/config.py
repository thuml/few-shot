...
cfg = CfgNode(dict(
    ...
    dataset=dict(
        ...
        base_val_file="",  # validation data on the base classes, if is None, not used
    ...
    train=dict(
        ...
        # for meta learning based methods
        meta=dict(
            n_support=5,
            n_query=16,
            num_episode=100,
    ...
    val=dict(
        ...
        freq=20,  # valid frequency, if <= 0, no val
        n_support=5,  # validate 5 shot accuracy
    ),
    test=dict(
        finetune_params=dict(
            iter=100,  # iter number of each fine-tune for each task
            optim="SGD",
            sgd_params=dict(
                lr=0.01,
                momentum=0.9,
    ...
    method=dict(
        ...
        backbone="resnet18",  # Conv4 / resnet12 / resnet18 / wrn_28_10,
        image_size=224,  # 80 / 84 / 224 / 80, corresponding to the backbone above
    ),
))

# load from file and override by command line arguments
